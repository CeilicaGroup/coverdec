...
Opcional: `LOG_LEVEL` (`info` en producción), `SOLVER_MAX_SECONDS` (límite CP-SAT, default `240` en el servicio Python), `SOLVER_NUM_WORKERS` (hilos de búsqueda OR-Tools, default `4`), `PLANNING_SOLVER_TIMEOUT_MS` (timeout HTTP Node hacia el solver; por defecto `max(240s, SOLVER_MAX_SECONDS + 60s)`).
